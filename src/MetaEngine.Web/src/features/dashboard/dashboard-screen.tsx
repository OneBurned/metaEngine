...
function calculationFailureMessage(run: CalculationRun) { const code = run.errorCode ?? "unknown_error"; const messages: Record<string, string> = { calculation_failed: "Внутренняя ошибка расчёта. Посмотрите logs API/Worker и повторите расчёт после исправления причины.", invalid_period: "Период расчёта задан неверно: дата окончания раньше даты начала.", portfolio_not_found: "Портфолио для расчёта не найдено в текущем workspace.", preset_not_found: "Пресет для расчёта не найден в текущем workspace.", unsupported_timeframe: "Выбранный таймфрейм нельзя посчитать из этого источника.", unknown_timeframe: "Выбранный таймфрейм не поддерживается.", transient_database_error: "Временная ошибка базы данных. Расчёт будет повторён автоматически или его можно повторить вручную.", unknown_error: "Backend не вернул код причины. Нужно смотреть logs API/Worker." }; return `${messages[code] ?? `Backend вернул код ошибки: ${code}.`} Код: ${code}.` }
