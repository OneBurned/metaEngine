...
const chartConfig = {
  ...
  localDrawdown: { label: "Local MDD", color: "#9333ea" },
  sourceZ: { label: "IN Z", color: "#7c3aed" },
  strategyZ: { label: "OUT Z", color: "#dc2626" },
...
function StrategyResultTable({ strategyType, metrics }: { strategyType: string | null; metrics: ReturnType<typeof deriveMetricSeries> }) {
  ...
  return <div className="rounded-lg border border-slate-200 bg-white"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-4 py-3"><div><p className="text-sm font-semibold text-slate-950">Таблица результата стратегии</p><p className="mt-1 text-xs text-slate-500">Показаны первые {rows.length.toLocaleString("ru-RU")} строк из {metrics.length.toLocaleString("ru-RU")}. Полный набор строк можно выгрузить через CSV.</p></div></div><div className="overflow-x-auto"><Table className="min-w-[1180px] text-xs"><TableHeader><TableRow><TableHead>Время</TableHead>{isMdd ? <><CommonDealHeads /><TableHead className="text-right">IN DD</TableHead><TableHead className="text-right">Local MDD</TableHead><DealResultHeads /></> : isZScore ? <><CommonDealHeads /><TableHead className="text-right">IN DD</TableHead><TableHead className="text-right">IN DD rolling mean</TableHead><TableHead className="text-right">IN DD rolling std</TableHead><TableHead className="text-right">IN Z</TableHead><DealResultHeads includeZScore /></> : <><TableHead className="text-right">IN Diff</TableHead><TableHead className="text-right">IN Accum</TableHead><TableHead className="text-right">RSI</TableHead><TableHead>Signal</TableHead><TableHead>Execution</TableHead><TableHead className="text-right">Weight</TableHead><TableHead className="text-right">OUT Diff</TableHead><TableHead className="text-right">OUT Accum</TableHead><TableHead className="text-right">OUT HWM</TableHead><TableHead className="text-right">OUT DD</TableHead><TableHead className="text-right">OUT MDD</TableHead></>}</TableRow></TableHeader><TableBody>{rows.map((point) => <StrategyResultRow key={point.timestamp} point={point} strategyType={strategyType} />)}</TableBody></Table></div>{visibleRows < metrics.length ? <div className="border-t border-slate-200 p-3 text-center"><Button type="button" variant="outline" size="sm" onClick={() => setVisibleRows((current) => Math.min(current + 500, metrics.length))}>Показать ещё {Math.min(500, metrics.length - visibleRows).toLocaleString("ru-RU")}</Button></div> : null}</div>
}
...
function DealResultHeads({ includeZScore = false }: { includeZScore?: boolean }) { return <><TableHead className="w-32">Signal</TableHead><TableHead className="w-36">Execution</TableHead><TableHead className="text-right">Active deals</TableHead><TableHead className="text-right">Weight</TableHead><TableHead className="text-right">OUT Diff</TableHead><TableHead className="text-right">OUT Accum</TableHead><TableHead className="text-right">OUT HWM</TableHead><TableHead className="text-right">OUT DD</TableHead>{includeZScore ? <><TableHead className="text-right">OUT DD rolling mean</TableHead><TableHead className="text-right">OUT DD rolling std</TableHead><TableHead className="text-right">OUT Z</TableHead></> : null}<TableHead className="text-right">OUT MDD</TableHead><TableHead className="text-right">Max config weight</TableHead><TableHead className="text-right">Max realized weight</TableHead></> }
function SignalCell({ value }: { value: unknown }) { const text = textField(value); if (!text) return <TableCell />; return <TableCell className="max-w-36 whitespace-normal break-words leading-5">{text.split(" - ").map((item, index) => <span key={`${item}-${index}`} className="block">{item}</span>)}</TableCell> }
...
function buildStrategyCsv(metrics: ReturnType<typeof deriveMetricSeries>, indicators: StrategyIndicatorPoint[], strategyType: string | null) {
  ...
  const indicatorColumns = strategyType === "rsi"
    ...
    : strategyType === "z_score"
      ? ["source_diff", "source_accum", "source_dd", "source_dd_mean", "source_dd_std", "source_z", "signal", "execution", "active_deals", "weight", "strategy_accum", "strategy_hwm", "strategy_dd", "strategy_dd_mean", "strategy_dd_std", "strategy_z", "strategy_mdd", "max_config_weight", "max_realized_weight"]
