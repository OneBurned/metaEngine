  ...
  return (
    <AppShell onSignOut={() => void signOut().then(() => navigate({ to: "/login" }))}>
      ...
      {workspace ? <>
        ...
        <section className="mt-7"><SectionTitle icon={<Play className="size-4" />} title="Запуски стратегий" /><StrategyRunTable runs={strategyRuns} selectedId={selectedRunId} onSelect={setSelectedRunId} onDelete={(run) => void handleDeleteStrategyRun(run)} canWrite={workspace.canWrite} savedRunIds={new Set(savedStrategies.map((strategy) => strategy.resultCalculationRunId))} presentationSources={presentationSources} /></section>
