...
export type MddOptimizationParameters = {
  levels: Array<{ drawdown: number; weight: number }>
  takeProfit: number
}
...
export function MddOptimizationPanel({
  ...
  const [takeProfitRange, setTakeProfitRange] = useState<OptimizationNumericRange>({ from: 0, to: 20, step: 1 })
...
function parseMddParameters(parametersJson: string): MddOptimizationParameters | null { try { const value = JSON.parse(parametersJson) as { levels?: unknown; takeProfit?: unknown }; if (!Array.isArray(value.levels) || typeof value.takeProfit !== "number") return null; const levels = value.levels.filter((level): level is { drawdown: number; weight: number } => typeof level === "object" && level !== null && typeof (level as { drawdown?: unknown }).drawdown === "number" && typeof (level as { weight?: unknown }).weight === "number"); return levels.length === value.levels.length ? { levels, takeProfit: value.takeProfit } : null } catch { return null } }
