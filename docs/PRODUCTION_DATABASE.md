...
Этот документ описывает PostgreSQL persistence foundation MetaEngine. На этапе
P1b база и migrations готовы, но production API workflows, авторизация и
расчетный движок еще не реализованы.
...
| --- | --- |
| `users` | Учетные записи и статус доступа; вход появится позже |
...
- добавить API workflows и обязательную workspace-проверку;
- подключить аутентификацию и роли;
