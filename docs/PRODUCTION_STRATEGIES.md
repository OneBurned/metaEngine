...
The manual strategy form and RSI/MDD optimization forms show completed base-calculation candidates with the compact calculation timestamp (`YYYY-MM-DD HH:MM`) so repeated runs with the same name and period can be distinguished. The result block also restores the trading-model chart from the old local lab:
