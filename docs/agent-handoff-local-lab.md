...
Use custom date controls for date/time inputs: a full-width text field in `YYYY-MM-DD HH:MM` format, a compact calendar button, and a custom popup with date/hour/minute plus `OK` and `Отмена`. Do not rely on native `datetime-local` as the only control; the user wants both manual paste/input and a picker. The chosen minutes must be preserved, with `00` only as the default when minutes are absent.
...
- uploaded CSV return series are called `portfolio/portfolios`, not `strategy/strategies`;
...
- same portfolio can be repeated for rebalancing if periods do not overlap;
- deleted/missing portfolio in a preset contributes zero diffs;
- re-uploading a portfolio with the same filename makes it active again;
- `samples/strategies` is reserved for trading strategy JSON configs;
- MDD is a series, not only one scalar;
- user wants simple language and exact commands after updates.

## 23.1 Current accepted workflow after a successful PR

When the user says the result is good and wants to “fix the result”:

1. Tell them to merge the current PR on GitHub with the real button names: `Merge pull request` and `Confirm merge`.
2. After merge, tell them to run from `main`:
...
3. If the user wants a backup of the now-working code, create a new backup branch and tag from `main` after `git pull`:

```bash
git branch backup/working-local-lab-vN-YYYY-MM-DD
git tag working-local-lab-vN
git push origin backup/working-local-lab-vN-YYYY-MM-DD
git push origin working-local-lab-vN
```

The user currently does not care if local CSV files are lost, unless they explicitly say otherwise. Do not overcomplicate normal update instructions with CSV backup commands.
...
- Date text must be fully visible as `YYYY-MM-DD HH:MM`; do not make fields so narrow that only `YYYY-MM-DD H` is visible.
- The calendar trigger should be compact, currently a calendar icon button, not a large `Выбрать` button that overlaps neighboring fields.
- Picker popup must have explicit `OK` and `Отмена`; `OK` belongs inside the custom popup.
