...
function convertRowsToTimeframe(sourceRows, from, to, sourceStep, targetTimeframe) {
  ...
  const diffs = boundaries.map((ts, index) => {
    ...
    const previousEquity = 1 + (previous?.accum ?? 0);
    return previousEquity === 0
      ? 0
      : (1 + (current?.accum ?? 0)) / previousEquity - 1;
  });
...
function calculateFromDiffs(grid, diffs) {
  ...
  for (let i = 0; i < grid.length; i += 1) {
    ...
    if (!Number.isFinite(diff)) throw new Error(`Доходность должна быть конечным числом в точке ${grid[i]}`);
    if (diff < -1) throw new Error(`Доходность не может быть меньше -100% в точке ${grid[i]}`);
...
function calculatePresetFromSources(preset, portfolioSources, periodFrom, periodTo, options = {}) {
  ...
  for (const item of preset.items ?? []) {
    const portfolio = itemPortfolio(item);
    const loaded = portfolioSources.get(portfolio);
    if (!loaded) {
      missingPortfolios.add(portfolio);
      continue;
    }
    steps.push(loaded.step);
    const firstTimestamp = loaded.points[0]?.timestamp ?? 0;
    sourcePhases.set(portfolio, ((firstTimestamp % loaded.step) + loaded.step) % loaded.step);
  }
  ...
  for (const item of preset.items ?? []) {
    ...
    for (let i = 0; i < grid.length; i += 1) {
      ...
      if (!active) continue;
      if (loaded && ((ts % loaded.step) + loaded.step) % loaded.step !== sourcePhases.get(portfolio)) continue;
  ...
  return {
    ...
    sourceTimeframe: timeframeFromStep(step),
    step: timeframe,
    sourceStep: step
  };
}
...
function calculatePreset(preset, portfoliosDir, periodFrom, periodTo, options = {}) {
  ...
  for (const item of preset.items ?? []) {
    ...
    if (fs.existsSync(portfolioPath)) {
      portfolioCache.set(portfolio, readPortfolioFile(portfolioPath));
    }
  }

  return calculatePresetFromSources(
    preset,
    portfolioCache,
    periodFrom,
    periodTo,
