...
function normalizeToDiffCsv(text, valueType) {
  const points = parseStrategyCsv(text);
...
function readStrategyFile(filePath) {
  ...
  const points = parseStrategyCsv(text).map((point) => ({ timestamp: point.timestamp, diff: point.value }));
...
function validatePresetItems(items) {
  ...
  for (const item of items) {
    ...
    if (!strategy) throw new Error('В строке пресета не выбрана стратегия');
    const from = parseTimestamp(item.date_from);
    const to = item.date_to ? parseTimestamp(item.date_to) : null;
    if (to !== null && to <= from) throw new Error(`У ${strategy} дата окончания должна быть позже даты начала`);
    if (!byStrategy.has(strategy)) byStrategy.set(strategy, []);
    byStrategy.get(strategy).push({ from, to });
  }

  for (const [strategy, ranges] of byStrategy.entries()) {
    ranges.sort((a, b) => a.from - b.from);
    for (let i = 1; i < ranges.length; i += 1) {
      const previousEnd = ranges[i - 1].to ?? Infinity;
      if (ranges[i].from < previousEnd) {
        throw new Error(`У стратегии ${strategy} пересекаются периоды в пресете`);
      }
    }
  }
}

function calculateStrategy(strategyPath, periodFrom, periodTo, forcedStep = null) {
  const { points, step } = readStrategyFile(strategyPath);
...
function calculatePreset(preset, strategiesDir, periodFrom, periodTo) {
  validatePresetItems(preset.items ?? []);
  const strategyCache = new Map();
  const steps = [];
  for (const item of preset.items ?? []) {
    const strategyPath = path.join(strategiesDir, item.strategy);
    if (!fs.existsSync(strategyPath)) throw new Error(`Файл стратегии не найден: ${item.strategy}`);
    const loaded = readStrategyFile(strategyPath);
    strategyCache.set(item.strategy, loaded);
  ...
  for (const item of preset.items ?? []) {
    const loaded = strategyCache.get(item.strategy);
    const map = new Map(loaded.points.map((point) => [point.timestamp, point.diff]));
    ...
    for (let i = 0; i < grid.length; i += 1) {
      ...
      if (rawDiff === undefined) {
        warnings.push({ strategy: item.strategy, timestamp: ts, display: formatTimestamp(ts), reason: 'missing_diff_zero' });
      }
