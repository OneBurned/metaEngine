...
function createMddRandomParameterGrid(ranges, options = {}) {
  ...
  const entryRanges = [1, 2, 3, 4, 5].map((index) => expandNumericRange(`entry${index}`, ranges?.[`entry${index}`]));
  const exitLevels = expandNumericRange('exitLevel', ranges?.exitLevel);
  const grid = [];
  const seen = new Set();
  const maxAttempts = maxCandidates * 20;

  for (let attempt = 0; grid.length < maxCandidates && attempt < maxAttempts; attempt += 1) {
    const entries = entryRanges.map((values) => randomChoice(values, random)).sort((a, b) => a - b);
    ...
    const key = `${entries.join('|')}|${exitLevel}`;
  ...
  if (!grid.length) throw new Error('MDD random search: не удалось создать кандидатов. Проверьте диапазоны входов.');
  return grid;
}
