...
function mddMinEntryDeltaFromRanges(ranges) {
  const value = Math.abs(Number(ranges?.minEntryDelta ?? 0));
  return Number.isFinite(value) ? value : 0;
}

function mddEntryRangesFromRanges(ranges, count) {
  if (ranges?.parameterMode === 'simple') {
    const shared = expandNumericRange('entry', ranges?.entry);
    return Array.from({ length: count }, () => shared);
  }
  return Array.from({ length: count }, (_, index) => expandNumericRange(`entry${index + 1}`, ranges?.[`entry${index + 1}`]));
}

function mddWeightRangesFromRanges(ranges, count) {
  if (ranges?.parameterMode === 'simple') {
...
function mddEntriesAreSeparated(entries, minEntryDelta) {
  return entries.every((value, index, values) => {
    if (index === 0) return true;
    const delta = value - values[index - 1];
    return minEntryDelta > 0 ? delta >= minEntryDelta : delta > 0;
  });
}

function mddWeightsAreNonDecreasing(weights) {
...
function createMddParameterGrid(ranges) {
  ...
  const minEntryDelta = mddMinEntryDeltaFromRanges(ranges);
  const entryRanges = mddEntryRangesFromRanges(ranges, count);
  const weightRanges = mddWeightRangesFromRanges(ranges, count);
  ...
  function walkWeights(entries, weights, index, totalWeight) {
    ...
    for (const weight of weightRanges[index]) {
      ...
      if (!mddWeightsAreNonDecreasing(candidate)) continue;
      const nextTotal = totalWeight + weight;
      if (nextTotal > maxTotalWeight + 1e-9) continue;
      walkWeights(entries, candidate, index + 1, nextTotal);
    }
  }
  ...
  function walkEntries(entries, index) {
    ...
    for (const entry of entryRanges[index]) {
      ...
      if (!mddEntriesAreSeparated(candidate, minEntryDelta)) continue;
      walkEntries(candidate, index + 1);
    }
  }
...
function createMddRandomParameterGrid(ranges, options = {}) {
  ...
  for (let attempt = 0; grid.length < maxCandidates && attempt < maxAttempts; attempt += 1) {
    ...
    if (!mddEntriesAreSeparated(entries, minEntryDelta)) continue;
    const weights = pairs.map((pair) => pair.weight);
    if (!mddWeightsAreNonDecreasing(weights)) continue;
